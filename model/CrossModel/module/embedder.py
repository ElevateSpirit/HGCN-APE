...
class BertAttention(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        batch_size, num_sents, token_len = input_ids.shape
        
        input_ids = input_ids.reshape(-1, token_len)
        attention_mask = attention_mask.reshape(-1, token_len)
        
        bert_feature = self.bert(input_ids, attention_mask)[0]
        bert_feature = bert_feature[:, :, :]
        attention_score = self.dropout(self.linear(bert_feature))
        attention_score = torch.softmax(attention_score, dim=1)
        bert_attention_feature = attention_score * attention_mask[:, :, None] * bert_feature
        bert_attention_feature = torch.sum(bert_attention_feature, dim=1)
        
        bert_attention_feature = bert_attention_feature.view(batch_size, num_sents, -1)
        
        return bert_attention_feature 